...
func TestClassifier_ScoreArticles(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if !assert.Len(t, body.Messages, 2) {
			return
		}
		...
		resp := openai.ChatCompletionResponse{
			Choices: []openai.ChatCompletionChoice{{
				Message: openai.ChatCompletionMessage{Content: `[{"guid":"item-a","score":7.5,"explanation":"looks relevant","topics":["golang","programming"],"summary":""}]`},
			}},
		}
		w.Header().Set("Content-Type", "application/json")
		assert.NoError(t, json.NewEncoder(w).Encode(resp))
	...
	classifications, err := classifier.ScoreArticles(context.Background(), ClassifyRequest{
		Articles: []domain.Item{{
			GUID:        "item-a",
			Title:       "Go 1.24 Released",
			Description: "Release notes",
			// content must be stripped by ScoreArticles so the prompt stays cheap
			Content: "FULL EXTRACTED BODY TEXT THAT MUST NOT LEAK INTO THE PROMPT",
		}},
	})
	require.NoError(t, err)
	require.Len(t, classifications, 1)
	assert.Equal(t, "item-a", classifications[0].GUID)
	assert.InDelta(t, 7.5, classifications[0].Score, 0.001)
	assert.Empty(t, classifications[0].Summary)

	// scoring phase uses the cheap system prompt, not the default summary-heavy one
	assert.Contains(t, capturedSystem, "only the title and description")
	assert.NotContains(t, capturedUser, "FULL EXTRACTED BODY TEXT")
	assert.Contains(t, capturedUser, "Go 1.24 Released")
}

func TestClassifier_SummarizeArticle(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		resp := openai.ChatCompletionResponse{
			Choices: []openai.ChatCompletionChoice{{
				Message: openai.ChatCompletionMessage{Content: `[{"guid":"item-b","score":9,"explanation":"deep dive on the topic","topics":["golang"],"summary":"Go 1.24 ships a revamped scheduler that improves tail latency on large thread pools. New toolchain command simplifies version management across monorepos. Compile times drop 20% through better parallelization. Runtime telemetry now exposes per-goroutine allocation rates without profiling overhead. Several stdlib packages move under the newer iter.Seq interfaces."}]`},
			}},
		}
		w.Header().Set("Content-Type", "application/json")
		assert.NoError(t, json.NewEncoder(w).Encode(resp))
	...
	cls, err := classifier.SummarizeArticle(context.Background(), domain.Item{
		GUID:    "item-b",
		Title:   "Go 1.24 Deep Dive",
		Content: "EXTRACTED_ARTICLE_BODY_MARKER",
	}, ClassifyRequest{})
	require.NoError(t, err)
	assert.Equal(t, "item-b", cls.GUID)
	assert.InDelta(t, 9.0, cls.Score, 0.001)
	assert.NotEmpty(t, cls.Summary)
	// summarize must include full content in the prompt
	assert.Contains(t, capturedUser, "EXTRACTED_ARTICLE_BODY_MARKER")
}
